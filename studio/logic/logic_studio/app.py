...
def apply_classic_style(app: QApplication) -> None:
    """The classic Windows 98/NT look (style + palette + QSS) this app
    has always launched with - extracted out of LogicStudioApp.__init__
    unchanged (Task: EPW Studio shell 2.3) so the shell can apply the
    identical look to an embedded MainWindow without constructing a
    second QApplication (only one may exist per process - QApplication
    itself is a singleton). Standalone `python main.py` still goes
