...
def test_canvas_second_line_shows_io_label_when_no_comment(qsettings):
    _app()
    item, block, window = _di_item_in_scene("ELA01.DI.1", qsettings)
    DeviceModel.set_io_label(window.project, "ELA01.DI.1", "Wyłącznik Q1 zamknięty")

    assert item._io_label_for_display("ELA01.DI.1") == "Wyłącznik Q1 zamknięty"
    window.close()
...
def test_canvas_second_line_prefers_comment_over_label(qsettings):
    ...
    label = item._io_label_for_display("ELA01.DI.1")
...
def test_canvas_falls_back_to_display_name_when_neither_set(qsettings):
    _app()
    item, block, window = _di_item_in_scene("ELA01.DI.2", qsettings)

    assert item._io_label_for_display("ELA01.DI.2") == ""
    comment = block.properties.get("Comment", "")
    label = item._io_label_for_display("ELA01.DI.2")
    second_line = comment or label or block.display_name
...
def test_signal_picker_opis_column_shows_label_when_set():
    ...
    found = _find_leaf_by_name(dialog.tree, "ELA01.DI.1")
...
def test_signal_picker_search_matches_label_text():
    ...
    other = _find_leaf_by_name(dialog.tree, "ELA01.DI.2")
