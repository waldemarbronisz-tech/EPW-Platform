...
export const HELP_CONTENT_CORE: HelpContentMap = {
  ...
  'dev-switched': {
    pl: [
      ...
      { kind: 'p', text: 'Liczba wyjsc (1 albo 2) i styl (MAINTAINED, PULSE albo PULSE_TOGGLE) razem opisuja realny uklad:' },
      { kind: 'list', items: [
      ...
      { kind: 'p', text: 'Przy 2 wyjsciach pole doOpen jest wymagane; przy 1 - zabronione. Przy stylu PULSE i PULSE_TOGGLE pole pulseMs (czas impulsu w milisekundach, > 0) jest wymagane; przy MAINTAINED - zabronione. PULSE_TOGGLE z trybem sprzezenia NONE jest bledem.' },
    ...
    en: [
      ...
      { kind: 'p', text: 'The output count (1 or 2) and the style (MAINTAINED, PULSE or PULSE_TOGGLE) together describe the real circuit:' },
      { kind: 'list', items: [
      ...
      { kind: 'p', text: 'With 2 outputs, doOpen is required; with 1, it is forbidden. With PULSE and PULSE_TOGGLE, pulseMs (the pulse time in milliseconds, > 0) is required; with MAINTAINED, it is forbidden. PULSE_TOGGLE with feedback mode NONE is an error.' },
