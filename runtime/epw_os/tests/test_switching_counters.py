...
def test_no_counting_on_the_very_first_observation(bus, mgr):
    ...
    bus.emit("tag_changed", "DI1", True, "GOOD")
    snap = mgr.get_snapshot("DI1")
...
def test_close_then_open_counts_each_separately(bus, mgr):
    bus.emit("tag_changed", "DI1", False, "GOOD")  # seed - not counted
    bus.emit("tag_changed", "DI1", True, "GOOD")    # -> closed: 1 close
    bus.emit("tag_changed", "DI1", False, "GOOD")   # -> open: 1 open
    snap = mgr.get_snapshot("DI1")
...
def test_repeated_writes_of_the_same_value_are_not_counted(bus, mgr):
    ...
    # switching event.
    bus.emit("tag_changed", "DI1", False, "GOOD")  # seed
    bus.emit("tag_changed", "DI1", True, "GOOD")   # 1 close
    bus.emit("tag_changed", "DI1", True, "GOOD")   # duplicate - ignored
    bus.emit("tag_changed", "DI1", True, "GOOD")   # duplicate - ignored
    snap = mgr.get_snapshot("DI1")
    assert snap["closes"] == 1
    assert snap["opens"] == 0
...
def test_bool_and_int_tag_values_are_both_understood(bus, mgr):
    bus.emit("tag_changed", "DI1", 0, "GOOD")  # seed as int
    bus.emit("tag_changed", "DI1", 1, "GOOD")  # close, as int
    snap = mgr.get_snapshot("DI1")
    assert snap["closes"] == 1


def test_multiple_cycles_accumulate(bus, mgr):
    bus.emit("tag_changed", "DI1", False, "GOOD")
    for _ in range(5):
        bus.emit("tag_changed", "DI1", True, "GOOD")
        bus.emit("tag_changed", "DI1", False, "GOOD")
    snap = mgr.get_snapshot("DI1")
...
def test_counters_are_independent_per_tag(bus, mgr):
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI2", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")
    assert mgr.get_snapshot("DI1")["closes"] == 1
    assert mgr.get_snapshot("DI2")["closes"] == 0


def test_non_digital_input_tags_are_ignored(bus, mgr):
    bus.emit("tag_changed", "DO01", False, "GOOD")
    bus.emit("tag_changed", "DO01", True, "GOOD")
    bus.emit("tag_changed", "Cabinet.Door", "CLOSED", "GOOD")
    assert mgr.get_snapshot("DO01") == new_record()
...
def test_non_boolean_int_values_on_a_di_tag_are_ignored(bus, mgr):
    ...
    bus.emit("tag_changed", "DI1", "not a bool", "GOOD")
    bus.emit("tag_changed", "DI1", None, "GOOD")
    snap = mgr.get_snapshot("DI1")
...
def test_closed_time_keeps_accruing_live_while_still_closed(bus, mgr):
    ...
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")
    first = mgr.get_snapshot("DI1")["closed_seconds"]
    time.sleep(0.15)
    second = mgr.get_snapshot("DI1")["closed_seconds"]
    assert second > first


def test_closed_time_does_not_grow_while_open(bus, mgr):
    bus.emit("tag_changed", "DI1", True, "GOOD")  # seed as closed - no count
    bus.emit("tag_changed", "DI1", False, "GOOD")  # -> open, 1 "open" recorded
    first = mgr.get_snapshot("DI1")["closed_seconds"]
    time.sleep(0.1)
    second = mgr.get_snapshot("DI1")["closed_seconds"]
...
def test_still_closed_across_a_restart_keeps_accruing_closed_time(bus, pm):
    ...
    mgr1 = SwitchingCounterManager(bus, pm)
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")  # now closed
    time.sleep(0.15)
    ...
    bus2.emit("tag_changed", "DI1", True, "GOOD")
    snap = mgr2.get_snapshot("DI1")
...
def test_corrupt_persisted_record_falls_back_to_a_fresh_one(bus):
    pm = FakeProjectManager(initial={"DI1": "not a dict", "DI2": {"closes": "abc"}})
    mgr = SwitchingCounterManager(bus, pm)  # must not raise
    assert mgr.get_snapshot("DI1") == new_record()
    assert mgr.get_snapshot("DI2") == new_record()
...
def test_reset_counter_zeroes_everything_but_keeps_the_threshold(bus, mgr):
    ...
    bus.emit("tag_changed", "DI1", True, "GOOD")
    bus.emit("tag_changed", "DI1", False, "GOOD")
    mgr.set_warning_threshold("DI1", 10000)

    old = mgr.reset_counter("DI1")
    assert old["closes"] == 1
    snap = mgr.get_snapshot("DI1")
    assert snap["closes"] == 0
    assert snap["opens"] == 0
...
def test_reset_while_currently_closed_keeps_tracking_from_now(bus, mgr):
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")  # currently closed
    mgr.reset_counter("DI1")
    time.sleep(0.1)
    snap = mgr.get_snapshot("DI1")
    assert snap["closes"] == 0
...
def test_reset_of_an_unknown_tag_does_not_raise(bus, mgr):
    old = mgr.reset_counter("DI99")
    assert old == new_record()
    assert mgr.get_snapshot("DI99") == new_record()
...
def test_no_threshold_by_default(bus, mgr):
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")
    assert mgr.is_over_threshold("DI1") is False


def test_threshold_triggers_once_reached(bus, mgr):
    mgr.set_warning_threshold("DI1", 3)
    bus.emit("tag_changed", "DI1", False, "GOOD")
    for _ in range(2):
        bus.emit("tag_changed", "DI1", True, "GOOD")
        bus.emit("tag_changed", "DI1", False, "GOOD")
    assert mgr.is_over_threshold("DI1") is False  # 2 closes so far
    bus.emit("tag_changed", "DI1", True, "GOOD")  # 3rd close
    assert mgr.is_over_threshold("DI1") is True


def test_threshold_can_be_cleared(bus, mgr):
    mgr.set_warning_threshold("DI1", 1)
    bus.emit("tag_changed", "DI1", False, "GOOD")
    bus.emit("tag_changed", "DI1", True, "GOOD")
    assert mgr.is_over_threshold("DI1") is True
    mgr.set_warning_threshold("DI1", None)
    assert mgr.is_over_threshold("DI1") is False


def test_unknown_tag_is_never_over_threshold(mgr):
    assert mgr.is_over_threshold("DI50") is False
